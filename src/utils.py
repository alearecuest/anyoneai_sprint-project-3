...
class ImageDownloader:
    ...
    - overwrite (bool): Whether to override existing images.
    ...
    def download_images(self, df, print_every=1000):
        """
        Download images from URLs in a DataFrame.

        Args:
        - df (pd.DataFrame): DataFrame containing image URLs in 'image' column and SKU in 'sku' column.
        - print_every (int): Print progress every n images.
        ...
        for index, row in df.iterrows():
            ...
            # Check if image already exists and overwrite is False
...
def train_test_split_and_feature_extraction(df, test_size=0.3, random_state=42):
    ...
    Args:
    - df (pd.DataFrame): DataFrame containing the data with embeddings and labels.

    Keyword Args:
    - test_size (float): Size of the test set (default: 0.3 for 30%).
    - random_state (int): Random state for reproducibility (default: 42).

    Returns:
    - train_df (pd.DataFrame): Training set DataFrame.
    - test_df (pd.DataFrame): Test set DataFrame.
    - text_columns (list): List of columns with text embeddings (text_1, text_2, etc.).
    - image_columns (list): List of columns with image embeddings (image_1, image_2, etc.).
    - label_columns (list): List of columns with class labels.

    Example:
    train_df, test_df, text_columns, image_columns, label_columns = train_test_split_and_feature_extraction(df)
    """

    # Split the data into train and test sets using stratified split on class_id
    train_df, test_df = train_test_split(
        df, test_size=test_size, random_state=random_state, stratify=df["class_id"]
    )

    # Select features and labels vectors:
    # Extract text embedding columns (columns starting with 'text_')
    text_columns = [col for col in df.columns if col.startswith("text_")]

    # Extract image embedding columns (columns starting with 'image_')
    image_columns = [col for col in df.columns if col.startswith("image_")]

    # Extract label column (class_id column represents the product category)
